    ...
    app.post('/api/chat', async (req, res) => {
        ...
        "https://openrouter.ai/api/v1/chat/completions",
        {
            ...
            {
                role: "system",
                content: `You are AstroBot, an expert space assistant embedded in AstroView — a real-time space intelligence dashboard. 
        Today's date is ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}.
        Answer questions about planets, ISS, asteroids, space weather, launches, and astronomy. Be concise and engaging.`
            },
        ...
        {
            headers: {
                "Authorization": "Bearer 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",
    ...
    // ── Nominatim reverse geocoding (CORS proxy) ─────────────────────────────
...
app.get('/api/space-weather/location', async (req, res) => {
    ...
    try {
        ...
        let cmeData = [];
        try {
            const cmeRes = await axios.get(
                `https://api.nasa.gov/DONKI/CME?startDate=${fmt(start)}&endDate=${fmt(end)}&api_key=${NASA_KEY}`
            );
    ...
    try {
        ...
        try {
            const neoRes = await axios.get(
                `https://api.nasa.gov/neo/rest/v1/feed?start_date=${fmt(start)}&end_date=${fmt(end)}&api_key=${NASA_KEY}`
            );
